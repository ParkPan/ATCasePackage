...
import pipeofodps
import comparefile
import createodpsdata

print 'perpare jar to odps'
# if not perparejar.gitpullcode(constvars.folderpath):
#     quit()
# if not perparejar.mvnpackage(constvars.folderpath):
#     quit()
...
# startodpsjob.perpareconfig(constvars.workpath, constvars.configpath)
# startodpsjob.dropodpsdata(constvars.PeakBandWidthMiddleTableJob['outputtable'],0)
# startodpsjob.startodpsjob(constvars.workpath, constvars.FiveMinutePeakBandWidthJob['name'], ['hour="true"','time=22'])
# comparefile.compareresultpatternmysql(constvars.FiveMinutePeakBandWidthJob['name'],
#                                        r'select five_minute_index, username, peak_band_width, play_type '
#                                    r'from %s where date_time="20160314"' % constvars.FiveMinutePeakBandWidthJob['mysqltable'] ,'.*%s.*', 2, 3)
