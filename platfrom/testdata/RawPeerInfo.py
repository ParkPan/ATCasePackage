...
class RawPeerInfo(dataprovider.Dataprovider):

    tablename = 'raw_input_peer_info'
        ...
        with open(os.path.abspath(os.path.dirname(__file__)) + '/RawPeerInfo.txt', 'w') as filedemanddata:
            for i in range(24):
                for j in range(60):
                    ...
                    id, int(timestamp),peerid,os_type,os_version,int(nat_type),publicip,macs,int(timestamp)+random.randint(10, 1000),int(timestamp)+random.randint(1001, 10000)))
