...
def sdk_check_request(pattern_dict, time_out):
    ...
    for p_key in pattern_dict.keys():
        res_list[re_const.PATTERN_NAME[p_key]] = [False, False]
    ...
    while True:
        ...
        for ret_text in txt_array:
            for pattern_key in pattern_dict.keys():
                ...
                if m:
                    res_list[re_const.PATTERN_NAME[pattern_key]][0] = True
                    break
                m = re.match(pattern_dict[pattern_key], ret_text)
                if m:
                    if res_list[re_const.PATTERN_NAME[pattern_key]][0]:
                        res_list[re_const.PATTERN_NAME[pattern_key]][1] = True  # pattern '' match always OK
...
if __name__ == "__main__":
    log_file = open(re_const.LOG_FILE, "w")
    timeout = 660
    if len(sys.argv) > 1:
        timeout = int(sys.argv[1])
    r_list = sdk_check_request(re_const.PATTERN_DICT, timeout)
